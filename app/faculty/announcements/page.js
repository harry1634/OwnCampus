  ...
  useEffect(() => {
    fetch('/api/announcements')
      ...
      .then(d => {
        ...
        setAnnouncements(all.filter(a => a.target_audience === 'all' || a.target_audience === 'faculty'))
      })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: '#64748B', marginTop: 3 }}>Institution-wide notices and updates</p>
      </div>
        ...
        {FILTERS.map(f => {
          ...
          return (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '6px 14px', borderRadius: 99, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `1px solid ${active ? (meta?.bg ?? '#D1FAE5') : '#E2E8F0'}`, background: active ? (meta?.bg ?? '#ECFDF5') : '#FFFFFF', color: active ? (meta?.color ?? '#059669') : '#64748B', transition: 'all 0.12s' }}>
      ...
      {fetchError && <p style={{ color: '#DC2626', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 8, padding: '10px 14px', fontSize: 13 }}>Error: {fetchError}</p>}
      {!loading && !fetchError && !filtered.length && (
