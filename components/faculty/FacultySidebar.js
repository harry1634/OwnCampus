...
const SB = '#064E3B'   // dark green — faculty portal
    ...
    <>
      ...
      <aside style={{
        ...
        <div style={{ height: 64, display: 'flex', alignItems: 'center', gap: 10, padding: showCollapsed ? '0 12px' : '0 12px 0 16px', borderBottom: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
          {showCollapsed ? (
            <button onClick={() => setCollapsed(false)} style={{ width: 40, height: 40, borderRadius: 10, background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.70)', cursor: 'pointer' }}
            ...
            <>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontWeight: 700, fontSize: 14, color: '#FFFFFF', lineHeight: 1.2, letterSpacing: '-0.01em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Faculty Portal</p>
              ...
              {!isMobile && (
                <button onClick={() => setCollapsed(true)} style={{ width: 24, height: 24, borderRadius: 6, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.50)', cursor: 'pointer', flexShrink: 0 }}
        ...
        {!showCollapsed && (
          <div style={{ padding: '10px 10px 6px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' }}>
              <div style={{ width: 26, height: 26, borderRadius: 7, background: cu.avatarUrl ? 'transparent' : 'rgba(255,255,255,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 11, color: '#FFFFFF', flexShrink: 0, overflow: 'hidden' }}>
                {cu.avatarUrl
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px' }} className="no-scrollbar">
          {NAV.map(group => (
            <div key={group.label} style={{ marginBottom: 12 }}>
              {!showCollapsed && (
                <p style={{ padding: '0 8px', marginBottom: 2, fontSize: 10, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)' }}>
                  {group.label}
                </p>
              )}
                  ...
                  return (
                    <Link key={item.name} href={item.href}
                      ...
                      style={{
                        ...
                        padding: showCollapsed ? '10px' : '7px 10px',
