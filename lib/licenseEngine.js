...
export async function getInstitutionLicense(institutionId) {
  ...
  const { data } = await admin
    ...
    .select('institution_id, valid_from, valid_until, max_students, max_faculty, max_admins, max_branches, max_departments, max_courses, max_classes, max_library_books, max_hostel_rooms, max_vehicles, max_transport_routes, max_api_requests, max_realtime_connections, max_storage_gb')
...
export async function checkHostelRoomLimit(institutionId) {
  ...
  return checkLimit(
    institutionId,
    async () => {
      // hostel_rooms has no institution_id — join via hostel_buildings
      const { data: buildings } = await admin.from('hostel_buildings')
        .select('id').eq('institution_id', institutionId)
      const ids = (buildings || []).map(b => b.id)
      if (!ids.length) return { count: 0 }
      return admin.from('hostel_rooms')
...
export async function getInstitutionUsage(institutionId) {
  ...
  // hostel_rooms has no institution_id — fetch building IDs first
  const { data: hostelBuildings } = await admin.from('hostel_buildings')
    .select('id').eq('institution_id', institutionId)
  const buildingIds = (hostelBuildings || []).map(b => b.id)
  ...
  const [
    ...
    admin.from('books').select('id', { count: 'exact', head: true })
      .eq('institution_id', institutionId).eq('is_active', true),
    buildingIds.length
      ? admin.from('hostel_rooms').select('id', { count: 'exact', head: true }).in('building_id', buildingIds)
