...
export function UserProvider({ profile: initialProfile, children }) {
  ...
  // _classSection / _roll are injected by the student layout from the students table;
  // fall back to metadata fields for non-student roles or legacy data.
  const [state, setState] = useState({
    name:         [initialProfile?.first_name, initialProfile?.last_name].filter(Boolean).join(' '),
    email:        initialProfile?.email || '',
    role:         initialProfile?.role  || '',
    userId:       initialProfile?.id    || '',
    classSection: initialProfile?._classSection || (meta.class ? `${meta.class}${meta.section ? '-' + meta.section : ''}` : ''),
    roll:         initialProfile?._roll || meta.roll_no || '',
