 ...
 * Pattern: Aa1!Bb2@Cc3#Dd4$ — satisfies most password policies.
 */
...
export async function provisionInstitution(institutionId, adminEmail, adminName, companyUserId = null) {
  ...
  if (authErr) {
    ...
    // User exists — look up by email and proceed
  ...
  await admin.from('user_profiles').upsert({
    ...
    is_active:      true,
    metadata:       { provisioned: true, institution_name: inst.name, temp_password: tempPassword },
  }, { onConflict: 'id' })
